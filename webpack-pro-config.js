...
module.exports = {
  entry: {
  // 文件入口配置
    index: './src/webs/' + webConfig.name + '/js/index',
    vendor: [
      'react-router',
      'react-redux',
      'redux'
  ...
  // 文件输出配置
    ...
    path: path.join(__dirname, 'src', 'webs', webConfig.name, 'dist'),
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
      template:'src/webs/' + webConfig.name + '/index.html',
      // html模板的路径
  ...
  module: {
    loaders: [
      ...
      {
        test: /\.scss$/,
        include: path.resolve(__dirname, 'src/webs/' + webConfig.name + '/js'),
        loaders: [
          'style',
          'css?modules&importLoaders=1&localIdentName=[local]___[hash:base64:5]',
          'postcss?parser=postcss-scss'
      ...
      {
        test: /\.scss$/,
        include: path.resolve(__dirname, 'src/webs/' + webConfig.name + '/styles'),
        loader: 'style!css!postcss?parser=postcss-scss'
      },
      // 项目公有样式，不需要私有化，单独配置
